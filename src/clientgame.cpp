...
int nextmode = 1;         // nextmode becomes gamemode after next map load
VAR(gamemode, 1, 0, 0);
...
void arenarespawn()
{
    ...
    else if(arenadetectwait==0 || arenadetectwait<lastmillis)
    {
        ...
        if(dead>0 && (alive<=1 || (m_team && oneteam)))
        {
...
{
    if(netmapstart() && m_sp) { gamemode = 1; conoutf("coop sp not supported yet"); };
    sleepwait = 0;
    monsterclear();
    projreset(); 
    spawncycle = -1;

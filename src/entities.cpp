...
void additem(int i, int &v, int spawnsec)
{
    ...
    {
        addmsg(1, 3, SV_ITEMPICKUP, i, m_sp ? 100000 : spawnsec);    // first ask the server for an ack
...
{
    loopv(ents) if((ents[i].type>=I_SHELLS && ents[i].type<=I_QUAD) || ents[i].type==TRIGGER)
    {
        putint(p, i);
        ents[i].spawned = true;
    };
